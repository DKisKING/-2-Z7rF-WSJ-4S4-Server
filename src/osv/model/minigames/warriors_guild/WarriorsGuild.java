package osv.model.minigames.warriors_guild;
	...
	public void handleDoor() {
		...
		//Walking into the cyclops room
			...
			if (player.getItems().playerHasItem(8851, 200)) {
				...
				if (current == -1) {
					player.getDH().sendNpcChat4("You are not in the possesion of a defender.", "You must kill cyclops to obtain a defender.",
							"The fee for entering the area is 200 tokens.", "Do you want to enter?", 2461, "Kamfreena");
					player.nextChat = 627;
				} else {
					player.getDH().sendNpcChat3("You are currently in posession of a " + ItemAssistant.getItemName(current) + ".",
							"It will cost 200 tokens to re-enter the cyclops area.", "Do you want to enter? It will cost you.", 2461, "Kamfreena");
					player.nextChat = 627;
				}
			} else {
				player.getDH().sendNpcChat2("You need atleast 200 warrior guild tokens.", "You can get some by operating the armour animator.", 2461, "Kamfreena");
				player.nextChat = 0;
			}
		}
	}
