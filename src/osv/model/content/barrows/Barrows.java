...
import osv.model.content.barrows.brothers.Ahrim;
import osv.model.content.barrows.brothers.Brother;
import osv.model.content.barrows.brothers.Dharok;
import osv.model.content.barrows.brothers.Guthan;
import osv.model.content.barrows.brothers.Karil;
import osv.model.content.barrows.brothers.Torag;
import osv.model.content.barrows.brothers.Verac;
...
public class Barrows {
	...
	public static final Boundary[] AREA = { GRAVEYARD, TOMBS, TUNNEL };
	...
	public Optional<Brother> getLastBrother() {
		return brothers.stream().filter(brother -> brother.isFinal()).findFirst();
	}
	...
	public void spawnBrother(int coffinId) {
		...
		brother.ifPresent(b -> b.handleSpawn());
	}
	 ...
	 */
	public void digDown() {
		brothers.stream().anyMatch(b -> b.digDown());
	}
	...
	public void moveUpStairs(int stairsId) {
		...
		brother.ifPresent(b -> b.moveUpStairs());
	}
	 ...
	 */
	public int countDeadBrothers() {
		return brothers.stream().filter(brother -> brother.isDefeated()).collect(Collectors.toList()).size();
	}
	...
	public void test() {
		Optional<Brother> brother = player.getBarrows().getBrother(1672);
		Optional<Brother> brother2 = player.getBarrows().getBrother(1673);
		Optional<Brother> brother3 = player.getBarrows().getBrother(1674);
		Optional<Brother> brother4 = player.getBarrows().getBrother(1675);
		Optional<Brother> brother5 = player.getBarrows().getBrother(1676);
		Optional<Brother> brother6 = player.getBarrows().getBrother(1677);
		completed = true;
		setCompleted(true);
		RewardList rewardList = new RewardList();
		setMonstersKilled(25);
		brother.get().handleDeath();
		brother2.get().handleDeath();
		brother3.get().handleDeath();
		brother4.get().handleDeath();
		brother5.get().handleDeath();
		brother6.get().handleDeath();
		//getBrothers().forEach(bro -> bro.setDefeated(true));
		for (int i = 0; i < 12; i++) {
			player.getPA().itemOnInterface(-1, -1, 64503, i);
		}
		brothers.stream().filter(b -> b.isDefeated()).forEach(br -> rewardList.addAll(br.getRewards()));
	 ...
	 */
	public void giveRewards(RewardList rewardList) {
		...
		if (Config.SERVER_STATE == ServerState.PUBLIC_PRIMARY) {
			if (sb.length() > 2) {
				Server.getBarrowsLogHandler().logReward(player, sb.toString().substring(0, sb.length() - 2), gotBarrows,
						getKillCount(), countDeadBrothers());
			}
		}
	}
	...
	private ArrayList<RewardItem> getWeightedRewards(RewardList rewardList, int amount) {
		ArrayList<RewardItem> randomRewards = new ArrayList<RewardItem>();
