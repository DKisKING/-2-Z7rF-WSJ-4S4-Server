...
public class Thieving {
	...
	public void steal(Stall stall, int objectId, Location3D location) {
		...
		int[] item = stall.items;
//		ItemDefinition definition = ItemDefinition.forId(itemToAdd);
		...
		int[] Gems = {1625, 1627, 1623, 1621, 1619, 1617};
		int itemToAdd = Gems[(getRandom(Gems.length-1))];
		player.getItems().addItem(itemToAdd,1);
...
//		player.sendMessage("You steal a " + definition.getName() + " from the stall.");
	...
	public enum Stall {
		CAKE(new int[] {1893}, 5, 16, 20, 45000),
		TEA(new int[] {712}, 5, 16, 10, 43000), 
		FUR(new int[] {6814}, 35, 36, 15, 42000), 
		SILVER(new int[] {2961}, 50, 54, 10, 40000), 
		GEM(new int[] {1625, 1627, 1623, 1621, 1619, 1617}, 75, 80, 10, 38000), 
		WINE(new int[] {1993}, 90, 90, 10, 36500);
		 ...
		 */
		private Stall(int[] items, int level, int experience, int depletionProbability, int petChance) {
			this.items = items;
			this.level = level;
			this.experience = experience;
