	...
	public static void attemptdata(final Player player, int npcId) {
		...
		for (int i = 0; i < data.length; i++) {
			if (npcId == data[i][0]) {
				...
				double experience = data[i][5] + percentOfXp; // XP
				...
				CycleEventHandler.getSingleton().addEvent(player, new CycleEvent() {
					...
					public void execute(CycleEventContainer container) {
						...
						if (player.playerSkillProp[10][1] > 0) {
							...
							if (Boundary.isIn(player, Boundary.RESOURCE_AREA)) {
								if (Misc.random(15) == 5) {
									...
									player.sendMessage("You received x" + randomAmount + " blood money while fishing!");
						...
						if (experience > 0) {
							player.getPA().addSkillXP((int) ((double) experience * (player.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.FISHING_EXPERIENCE)), player.playerFishing, true);
						}
