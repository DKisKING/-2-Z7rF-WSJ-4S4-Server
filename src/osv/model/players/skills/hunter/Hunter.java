package osv.model.players.skills.hunter;
...
import osv.model.players.skills.hunter.trap.Trap;
import osv.model.players.skills.hunter.trap.Trap.TrapState;
import osv.model.players.skills.hunter.trap.Trap.TrapType;
import osv.model.players.skills.hunter.trap.TrapProcessor;
import osv.model.players.skills.hunter.trap.TrapTask;
...
public final class Hunter {
	...
	public static boolean lay(Player player, Trap trap) {
		...
		if(GLOBAL_TRAPS.get(player).getTraps().size() >= getMaximumTraps(player)) {
			player.sendMessage("You cannot lay more then " + getMaximumTraps(player) + " with your hunter level.");
			return false;
		}
