...
public class Fletching {
	...
	public void fletchDart(int dartId) {
		...
		dart.ifPresent(d -> {
			...
			if (!player.getItems().playerHasItem(d.getId(), 15)) {
				player.sendMessage("You need at least 15 dart tips to do this.");
				return;
			}
			if (player.getItems().freeSlots() < 1) {
				player.sendMessage("You need at least 1 free slot to do this.");
			...
			player.getItems().deleteItem2(314, 15);
			player.getItems().deleteItem2(d.getId(), 15);
			player.getItems().addItem(d.getReward(), 15);
			player.getPA().addSkillXP((int) d.getExperience() * (player.getMode().getType().equals(ModeType.OSRS) ? 1 : Config.FLETCHING_EXPERIENCE), Skill.FLETCHING.getId(), true);
		});
	}
