...
import osv.model.players.combat.effects.ToxicBlowpipeEffect;
import osv.model.players.combat.effects.ToxicStaffOfTheDeadEffect;
import osv.model.players.combat.effects.TridentOfTheSwampEffect;
...
import osv.model.players.skills.mining.Pickaxe;
	...
	public static void attackNpc(Player c, int i) {
		...
		if (NPCHandler.npcs[i] != null) {
			...
			if (c.attackTimer <= 0) {
				...
				if (!PathChecker.isProjectilePathClear(c.absX, c.absY, c.heightLevel, theNPC.absX, theNPC.absY) && !Boundary.isIn(c, Boundary.PEST_CONTROL_AREA)) {
					c.attackTimer = 1;
					return;
				}
