package osv.model.players.combat.magic;
...
public class MagicMaxHit {
	...
	public static int magiMaxHit(Player c) {
		...
		if (c.playerEquipment[c.playerAmulet] == 12002 && c.playerEquipment[c.playerWeapon] != 6914 && c.playerEquipment[c.playerWeapon] != 11791
			...
			damageMultiplier += .15;
		} else if (c.playerEquipment[c.playerAmulet] == 12002 && c.playerEquipment[c.playerWeapon] == 6914) {
			damageMultiplier += .12;
		}
		if (c.npcIndex > 0 && c.getSlayer().getTask().isPresent()) {
			NPC npc = NPCHandler.npcs[c.npcIndex];
