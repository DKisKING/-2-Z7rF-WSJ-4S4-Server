...
import osv.Config;
import osv.Server;
import osv.model.content.PlayerEmotes;
...
import osv.model.content.staff.StaffControl;
import osv.model.content.staff.StaffInterface;
import osv.model.content.teleportation.CityTeleports;
...
import osv.model.players.combat.magic.LunarSpells;
...
import osv.model.players.packets.dialogueoptions.FiveOptions;
import osv.model.players.packets.dialogueoptions.FourOptions;
import osv.model.players.packets.dialogueoptions.ThreeOptions;
import osv.model.players.packets.dialogueoptions.TwoOptions;
...
import osv.model.players.skills.crafting.CraftingData.tanningData;
...
public class ClickingButtons implements PacketType {
	...
	public void processPacket(final Player c, int packetType, int packetSize) {
		...
		if (c.getRights().isOrInherits(Right.OWNER)) {
			Misc.println(c.playerName + " - actionbutton: " + actionButtonId + ", dialogaction: " + c.dialogueAction);
		}
		...
		if(TeleportationInterface.actions(c, actionButtonId)) {
			return;
		}
		...
		if (actionButtonId >= 175205 && actionButtonId <= 176149) {
			...
			if (!StaffControl.isUsingControl) {
				for (int i = 1; i < Config.MAX_PLAYERS; i++) {
					...
					if (!c.validClient(i))
						continue;
					if (actionButtonId == id + i) {
						if (p.playerName != null) {
							// c.sendMessage("Name: " + p.playerName);
...
//		}
		...
		if (c.rottenPotatoOption != "") {
			RottenPotato.optionActions(c, actionButtonId);
		}
		...
		case 113234://Rank
			...
			c.getRights().getSet().forEach(id -> sb.append("@blu@" + id + ", "));
			c.sendMessage("@cr10@Ranks: " + sb.substring(0, sb.length() - 1));
			break;
...
//				}
		...
		case 141011:
		case 140174:
			StaffInterface.kickPlayer(c);
			break;
			
		case 140172:
			StaffInterface.checkLootingBag(c);
			break;
			
		case 140173:
			StaffInterface.checkRunePouch(c);
			break;
			
		case 140170: //Zulrah
			StaffInterface.checkZulrah(c);
			break;
			
		case 140171: //Cerberus
			StaffInterface.checkCerberus(c);
			break;
			
		case 140168: //Check bank
			StaffInterface.checkBank(c, 0);
			break;
			
		case 141016:
			StaffInterface.checkBank(c, StaffInterface.bankTabId == 8 ? 0 : StaffInterface.bankTabId + 1);
			break;
			
		case 141017:
			StaffInterface.checkBank(c, StaffInterface.bankTabId == 0 ? 8 : StaffInterface.bankTabId - 1);
			break;
			
		case 141010: //Back
			StaffInterface.checkInfo(c);
			break;
			
		case 140169:
			StaffInterface.checkSafeBox(c);
			break;

		case 20174:
			c.getPA().closeAllWindows();
			osv.model.items.bank.BankPin pin = c.getBankPin();
		...
		case 226162:
			...
			if (!c.isBanking)
				return;
			if (System.currentTimeMillis() - c.lastBankDeposit < 1000)
				return;
...
//			c.getDH().sendDialogues(12000, -1);
		...
		case 113239:
			...
			c.forcedChat("I've played for: " + time);
			break;
		case 113240:
			break;
		case 113241:
		...
		case 113245:
			break;
		case 113247:
			c.didYouKnow = !c.didYouKnow;
			c.getPA().sendFrame126("@or1@- Did you know? @whi@" + (c.didYouKnow ? "Enabled" : "Disabled"), 29175);
			break;
		case 113249:
			break;
		case 10253:
...
//			break;
		...
		case 108006: // items kept on death
			...
			c.getPA().sendFrame126("Tyras - Items Kept on Death", 17103);
...
//		case 51023:
//		case 117210:
//			c.getDH().sendOption5("Chickens", "Cows", "Rock Crabs", "", "");
//			c.teleAction = 15;
//			c.dialogueAction = -1;
//			break;
//		case 51031:
...
//		case 4140: //Varrock
//		case 117112:
//			c.getDH().sendOption5("44 Portals", "West Drags", "Hill Giants", "East Drags", "");
//			c.teleAction = 1;
//			break;
//
//		case 4143:
