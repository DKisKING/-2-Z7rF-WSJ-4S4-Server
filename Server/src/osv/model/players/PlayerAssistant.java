...
import osv.model.content.instances.InstancedArea;
...
import osv.model.holiday.halloween.HalloweenDeathCycleEvent;
...
import osv.model.players.combat.effects.DragonfireShieldEffect;
import osv.model.players.combat.magic.MagicData;
import osv.model.players.combat.magic.NonCombatSpells;
...
import osv.model.players.skills.SkillHandler;
import osv.model.players.skills.Smelting;
import osv.model.players.skills.Smelting.Bars;
import osv.model.players.skills.crafting.CraftingData;
import osv.model.players.skills.crafting.Enchantment;
...
import osv.net.outgoing.messages.ComponentVisibility;
...
public class PlayerAssistant {
	...
	public void destroyInterface(int itemId, String config) {// Destroy item created by Remco
	...
	public void sendAutocastState(boolean state) {
	...
	public void magicOnItems(int slot, int itemId, int spellId) {
		...
		case 1178: // high alch
			...
			if (c.showDropWarning()) {
				c.getPA().destroyInterface(itemId, "alch");
				return;
			}
			alchemy(itemId, "high");
			break;
	...
	public int[][] boltData = { { 1155, 879, 9, 9236 }, { 1155, 9337, 17, 9240 }, { 1165, 9335, 19, 9237 }, { 1165, 880, 29, 9238 }, { 1165, 9338, 37, 9241 },
		...
		for (int i = 0; i < boltData.length; i++) {
			if (spell == boltData[i][0]) {
				if (bolt == boltData[i][1]) {
					...
					c.getPA().addSkillXP(boltData[i][2] * amount, 6, true);
					...
					if (c.killerId != c.getIndex()) {
						if (c.inWild()) {
							...
							if (!playerKiller.getPlayerKills().killedRecently(c.connectedFrom) && !playerKiller.getMacAddress().equals(c.getMacAddress())) {
								...
								playerKiller.getPA().sendFrame126("@or1@PK Points: @gre@" + playerKiller.pkp, 29163);
								playerKiller.getPA().sendFrame126("@or1@Kills/Deaths: @gre@" + playerKiller.killcount + "@or1@/@gre@" + playerKiller.deathcount, 29164);
								...
								if (opponentKillstreak > 1) {
									if (playerKiller.inClanWars() || playerKiller.inClanWarsSafe()) {
										...
										playerKiller.sendMessage("You receive an additional " + bloodMoneyAmount + " blood money, your opponent had a killstreak of " + opponentKillstreak + ".");
										PlayerHandler.executeGlobalMessage("<col=CC0000>" + WordUtils.capitalize(playerKiller.playerName) + "</col><col=255>" + " has ended </col><col=CC0000>" + WordUtils.capitalize(c.playerName) + "</col><col=255>'s" + " killstreak of " + opponentKillstreak + ".");
										PlayerHandler.executeGlobalMessage("<col=255>They have been awarded " + bloodMoneyAmount + " blood money.");
										//playerKiller.getItems().addItemUnderAnyCircumstance(13307, bloodMoneyAmount);
										if (!playerKiller.getItems().addItem(13307, bloodMoneyAmount)) {
											Server.itemHandler.createGroundItem(playerKiller, 13307, playerKiller.getX(), playerKiller.getY(), playerKiller.heightLevel, bloodMoneyAmount);
										}
									} else {
										playerKiller.sendMessage("You receive an additional 5 PK tickets, your opponent had a killstreak of " + opponentKillstreak + ".");
										PlayerHandler.executeGlobalMessage("<col=CC0000>" + WordUtils.capitalize(playerKiller.playerName) + "</col><col=255>" + " has ended </col><col=CC0000>" + WordUtils.capitalize(c.playerName) + "</col><col=255>'s" + " rogue killstreak of " + opponentKillstreak + ".");
										PlayerHandler.executeGlobalMessage("<col=255>They have been awarded " + opponentKillstreak + " pk tickets.");
										playerKiller.getItems().addItemUnderAnyCircumstance(2996, Config.DOUBLE_PKP ? opponentKillstreak * 2 : opponentKillstreak);
									}
								}
								...
								if (playerKiller.inClanWars()) {
									...
									playerKiller.pkp += Config.BONUS_WEEKEND && !Config.DOUBLE_PKP ? 5 : Config.DOUBLE_PKP ? 6 : 3;
								}
	...
	public void giveLife() {
		...
		c.getPA().sendFrame126("@or1@Kills/Deaths: @gre@" + c.killcount + "@or1@/@gre@" + c.deathcount, 29164);
		
		//If a player is in any of these areas, their items will not be dropped to themselves nor others
		if (!c.inDuelArena() && !Boundary.isIn(c, Boundary.DUEL_ARENA) && !Boundary.isIn(c, Boundary.FIGHT_CAVE)
				&& !Boundary.isIn(c, Boundary.LIGHTHOUSE) && !Boundary.isIn(c, PestControl.GAME_BOUNDARY)
				&& !c.inSafemode() && !Boundary.isIn(c, Zulrah.BOUNDARY)
				&& !Boundary.isIn(c, Boundary.CERBERUS_BOSSROOMS) && !Boundary.isIn(c, Boundary.RFD)) {
			...
			//If any items are on this list, delete them
				...
				//If ultimate ironman, delete all items
				if (c.getMode().isUltimateIronman()) {
	...
	public void followPlayer() {
		...
		if (otherX == c.absX && otherY == c.absY) {
			...
			for (int i = 0; i < nondiags.length; i++) {
				int x2 = otherX + nondiags[i][0];
				int y2 = otherY + nondiags[i][1];
	...
	public boolean addSkillXP(int amount, int skill, boolean dropExperience) {
		...
		if (Boundary.isIn(c, Boundary.DONATOR_ZONE) && c.getRights().isOrInherits(Right.VIP) && !Config.BONUS_WEEKEND) {
		...
		} else if (Boundary.isIn(c, Boundary.DONATOR_ZONE) && c.getRights().isOrInherits(Right.VIP) && Config.BONUS_WEEKEND) {
		...
		} else if (Config.BONUS_XP_WOGW && Config.BONUS_WEEKEND) {
			amount *= Config.SERVER_EXP_BONUS_BOOSTED;
		//Else (20%)
		} else if (Config.BONUS_XP_WOGW && c.bonusXpTime == 0 && !Config.BONUS_WEEKEND) {
			amount *= Config.SERVER_EXP_BONUS_WEEKEND;
	...
	public void addStarter() {
		...
		PlayerHandler.executeGlobalMessage("[@blu@<img=10>New Player@bla@] " + Misc.capitalizeJustFirst(c.playerName) + " @bla@has logged in! Welcome!");
		if (occurances < 2) {
			c.getItems().addItem(995, 750000);
	...
	public static boolean isSkillAreaItem(int item) {
