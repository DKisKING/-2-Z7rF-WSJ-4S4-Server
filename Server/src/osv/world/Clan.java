...
public class Clan {
		...
		for (int j = 0; j < PlayerHandler.players.length; j++) {
			if (PlayerHandler.players[j] != null) {
				...
				if ((c != null) && (this.activeMembers.contains(c.playerName)))
					c.sendMessage("/@bla@[@blu@" + getTitle() + "@bla@] <clan=" + getRank(paramClient.playerName) + ">@bla@" + Misc.optimizeText(paramClient.playerName) + ": @dre@"
							+ paramString + "");
			}
		}
	}
	...
	public void handleLootShare(Player c, int item, int amount) {
		...
		if (c.getShops().getItemShopValue(item, 1, c.getItems().getItemSlot(item)) > 1) {
			sendLootShareMessage(c.playerName + " has received " + amount + "x " + osv.model.items.Item.getItemName(item) + ".");
		}
	}
